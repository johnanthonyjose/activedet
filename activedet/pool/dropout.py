...
def _patch_dropout_layers(module: torch.nn.Module) -> bool:
    ...
    for name, child in module.named_children():
        if isinstance(child, torch.nn.Dropout):
            new_module = MCDropout(p=child.p, inplace=child.inplace)
        elif isinstance(child, torch.nn.Dropout2d):
            new_module = MCDropout2d(p=child.p, inplace=child.inplace)
        else:
            new_module = None
        ...
        child_changed = _patch_dropout_layers(child)
        changed = changed or child_changed
    return changed

def _unpatch_dropout_layers(module: torch.nn.Module) -> bool:
    """
    Recursively iterate over the children of a module and undo replacement if
    they are a dropout layer. This function operates in-place.
    ...
    for name, child in module.named_children():
        if isinstance(child, MCDropout):
            new_module = torch.nn.Dropout(p=child.p, inplace=child.inplace)
        elif isinstance(child, MCDropout2d):
            new_module = torch.nn.Dropout2d(p=child.p, inplace=child.inplace)
        else:
            new_module = None
        ...
        # recursively apply to child
        child_changed = _unpatch_dropout_layers(child)
        changed = changed or child_changed
    return changed


@contextmanager
def MCdropout_context(model):
    
    ret = _patch_dropout_layers(model)
    yield model
    ret2 = _unpatch_dropout_layers(model)
